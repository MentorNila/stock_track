...
class FormFiveGenerator {
    ...
    public function generateXml(){
        ...
        $this->writer = ($this->service->write('ownershipDocument', function ($writer) {
            ...
            $writer->writeElement('documentType', '5');
            if(isset($this->data['general-data']['event-date']) && !empty($this->data['general-data']['event-date'])){
                $this->data['general-data']['event-date'] = strtotime($this->data['general-data']['event-date']);
                $this->data['general-data']['event-date'] = date("Y-m-d", $this->data['general-data']['event-date']);
                $writer->writeElement('periodOfReport', $this->data['general-data']['event-date']);
            }
            $writer->writeElement('notSubjectToSection16', $this->data['general-data']['section-16']);
            $writer->writeElement('form3HoldingsReported', $this->data['general-data']['form-3']);
            $writer->writeElement('form4TransactionsReported', $this->data['general-data']['form-4']);
                    ...
                    foreach ($this->data['non-derivative'] as $nonDerivative) {
                        $writer->writeElement('nonDerivativeTransaction', function ($writer) use ($nonDerivative){
                            ...
                            $writer->writeElement('transactionCoding', function ($writer) use ($nonDerivative){
                                $writer->writeElement('transactionFormType', '5');
                            ...
                            $writer->writeElement('transactionAmounts', function ($writer) use ($nonDerivative){
                                ...
                                if((isset($nonDerivative['price'])) || isset($nonDerivative['price-num']) && !empty($nonDerivative['price-num'])){
                                    $writer->writeElement('transactionPricePerShare', function ($writer) use ($nonDerivative){
                                        if(isset($nonDerivative['price'])){
                                ...
                                if((isset($nonDerivative['price']) && !empty($nonDerivative['ad'])) || isset($nonDerivative['ad-num']) && !empty($nonDerivative['ad-num'])){
                            ...
                            $writer->writeElement('postTransactionAmounts', function ($writer) use ($nonDerivative){
                                if((isset($nonDerivative['secben']) && !empty($nonDerivative['secben'])) || isset($nonDerivative['secben-num']) && !empty($nonDerivative['secben-num'])){
                                    $writer->writeElement('sharesOwnedFollowingTransaction', function ($writer) use ($nonDerivative){
                                        if(isset($nonDerivative['secben']) && !empty($nonDerivative['secben'])){
                    ...
                    foreach ($this->data['derivative'] as $derivative) {
                        $writer->writeElement('derivativeTransaction', function ($writer) use ($derivative){
                            ...
                            $writer->writeElement('transactionAmounts', function ($writer) use ($derivative){
                                ...
                                if((isset($derivative['derivative'])) || isset($derivative['derivative-num']) && !empty($derivative['derivative-num'])){
