...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.filingData.title_singular') }}
    </div>

    <div class="card-body">
        <form action="{{ route("filing-datas.store") }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="form-group {{ $errors->has('code') ? 'has-error' : '' }}">
                <label for="code">{{ trans('cruds.filingData.fields.code') }}*</label>
                <input type="text"
                       id="code" name="code" class="form-control" value="{{ old('code', isset($filingData) ? $filingData->code : '') }}" required>
                @if($errors->has('code'))
