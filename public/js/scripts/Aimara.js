...
function createTree(p_div,p_backColor,p_contextMenu) {
	var tree = {
		...
		createNode: function(p_text,p_expanded, p_icon, p_parentNode,p_tag,p_contextmenu,node_id, has_children = true, p_code = '') {
			v_tree = this;
			node = {
				...
				createChildNode: function(p_text,p_expanded,p_icon,p_tag,p_contextmenu, node_id, has_children = false, p_code)  { return v_tree.createNode(p_text,p_expanded,p_icon,this,p_tag,p_contextmenu,node_id,has_children, p_code); }
      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node) {
			...
			if (p_node.childNodes.length == 0) {
			    ...
			    //TODO: image collapse, comment out visibility
                if (p_node.has_children){
                    v_exp_col = createImgElement('toggle_off','exp_col','/images/expand.png');
		...
		toggleNode: function(p_node) {
		    if (p_node.childNodes.length == 0 ){
                p_node = getChildNodes(p_node);
            }
		    p_node.childNodes.length=2
    ...
    $.ajax({
        ...
        success: function(parents) {
            $.each(parents, function(index, element) {
                node.createChildNode(element['label'], false, '/images/monitor.png', null, 'context1', element['id'], element['has_children'], element['code']);
            });
        }
    });
